...
<body>
	...
	<div class="content">
	Hello <?php echo $_SESSION['Forename']; ?>

		<form method="post" action="player-edit.php">
			<button type="submit" name="SetOwnPW" value="<?php echo $_SESSION['ID']; ?>">Reset my password</button>
			<button type="submit" name="Add" value="0">Add Player</button>
			<?php
	...
	if ($PlayerResult = $MySQLDB->query($SQLQuery))
	{
		if ($PlayerResult->num_rows > 0)
		{
			...
			while($PlayerRow = $PlayerResult->fetch_assoc())
			{
				?>
				<tr>
				<td><img src="<?php echo $PlayerRow['ImagePath']; ?>" height="32"></td>
				<td><?php echo $PlayerRow['Forename']; ?></td>
				<td><?php echo $PlayerRow['Surname']; ?></td>
				<td><?php $PlayerRow['EMail']; ?></td>
				<td><button type="submit" name="Edit" value="<?php echo $PlayerRow['ID']; ?>">Edit</button></td>
				<td><button type="Submit" name="Delete" value="<?php echo $PlayerRow['ID']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">Delete</button></td>
				</tr>
				<?php 
			}
			echo '</table>';
